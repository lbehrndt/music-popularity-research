...
# Perform polynomial regression for each year
for year in years:
    ...
    if len(X) > 10:  # Ensure enough data points for regression
        ...
        model.fit(X_poly, y)
        importance = np.abs(model.coef_[:len(features)])  # Extract first-order feature importances
        r2_scores.append(r2_score(y, model.predict(X_poly)))
...
plt.xlabel("Year")
plt.ylabel("R² Score")
plt.title("Polynomial Regression Model Accuracy Over Years")
